...
import { 
  ...
  Camera, Video, HardDrive, Copy, Check, Users, PhoneCall, X, Folder, UserPlus, HardDriveDownload, Receipt, Printer
} from 'lucide-react';
...
const ClientDetail: React.FC<ClientDetailProps> = ({
  ...
  return (
    <div className="space-y-5 animate-in fade-in duration-300">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-5 relative z-10">
          ...
          <div className="flex flex-wrap items-center gap-2.5 w-full lg:w-auto">
            ...
            <button
              ...
              className="flex-1 sm:flex-none items-center justify-center gap-1.5 px-3.5 py-2 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition-all text-xs flex shadow-xs"
              title="Download / Print Client Brief Portfolio PDF"
            >
              <Printer size={16} />
              Print / Save PDF
            </button>
      ...
      {/* ========================================================= */}
      {/* PRINT-ONLY NEW THEMED STRUCTURED CLIENT PORTFOLIO SHEET   */}
      {/* ========================================================= */}
      <div className="hidden print:block font-sans text-slate-900 bg-white p-8 space-y-6">
        {/* Document Header */}
        <div className="border-b-2 border-slate-900 pb-4 flex justify-between items-end">
          <div>
            <h1 className="text-2xl font-black tracking-tight text-slate-900">CONVEX ENTERTAINMENTS</h1>
            <p className="text-xs font-bold uppercase tracking-wider text-slate-500 mt-0.5">
              Official Client Production Brief & Shoot Roster Portfolio
            </p>
          </div>
          <div className="text-right text-xs">
            <span className="font-extrabold text-slate-900 block">Date Generated: {new Date().toLocaleDateString()}</span>
            <span className="text-slate-500 font-bold">Status: {client.status} Work</span>
          </div>
        </div>

        {/* Client Profile Summary Box */}
        <div className="grid grid-cols-2 gap-4 p-4 border border-slate-300 rounded-xl bg-slate-50 text-xs">
          <div>
            <h3 className="text-base font-black text-slate-900 mb-1">{client.name}</h3>
            <p className="mb-0.5"><strong>Tradition:</strong> {client.religion} Tradition</p>
            <p className="mb-0.5"><strong>Work Scope:</strong> {client.workScope === 'Single' ? 'Single Side' : 'Both Sides (Bride & Groom)'}</p>
          </div>

          <div>
            <p className="mb-0.5"><strong>Phone:</strong> {client.phone}</p>
            <p className="mb-0.5"><strong>Email:</strong> {client.email || 'N/A'}</p>
            {client.brideInstagram && <p className="mb-0.5"><strong>Bride Insta:</strong> {client.brideInstagram}</p>}
            {client.groomInstagram && <p className="mb-0.5"><strong>Groom Insta:</strong> {client.groomInstagram}</p>}
          </div>
        </div>

        {/* 1. Scheduled Function Shoots Table */}
        <div className="space-y-2">
          <h3 className="text-xs font-black text-slate-900 uppercase tracking-wider border-b border-slate-300 pb-1">
            1. Scheduled Function Shoots ({client.events.length})
          </h3>
          <table className="w-full text-xs text-left border border-slate-300">
            <thead className="bg-slate-100 font-bold border-b border-slate-300">
              <tr>
                <th className="p-2 border-r border-slate-300">Ceremony</th>
                <th className="p-2 border-r border-slate-300">Date & Time</th>
                <th className="p-2 border-r border-slate-300">Venue / Location</th>
                <th className="p-2">Side Scope</th>
              </tr>
            </thead>
            <tbody>
              {client.events.map(ev => (
                <tr key={ev.id} className="border-b border-slate-200">
                  <td className="p-2 font-bold border-r border-slate-200">{ev.type}</td>
                  <td className="p-2 border-r border-slate-200">{ev.date} {ev.time ? `@ ${ev.time}` : ''}</td>
                  <td className="p-2 border-r border-slate-200">{ev.venue || 'TBD'}</td>
                  <td className="p-2">{ev.sideType || 'Both'}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {/* 2. Assigned Shoot Crew Roster Table */}
        <div className="space-y-2">
          <h3 className="text-xs font-black text-slate-900 uppercase tracking-wider border-b border-slate-300 pb-1">
            2. Assigned Shoot Crew Roster
          </h3>
          <table className="w-full text-xs text-left border border-slate-300">
            <thead className="bg-slate-100 font-bold border-b border-slate-300">
              <tr>
                <th className="p-2 border-r border-slate-300">Ceremony</th>
                <th className="p-2 border-r border-slate-300">Crew Member</th>
                <th className="p-2 border-r border-slate-300">Assigned Role</th>
                <th className="p-2 border-r border-slate-300">Side</th>
                <th className="p-2">Contact Phone</th>
              </tr>
            </thead>
            <tbody>
              {client.events.flatMap(ev => (ev.crew || []).map(cr => ({ ...cr, eventName: ev.type }))).map((cr, idx) => (
                <tr key={idx} className="border-b border-slate-200">
                  <td className="p-2 font-bold border-r border-slate-200">{cr.eventName}</td>
                  <td className="p-2 border-r border-slate-200 font-extrabold">{cr.name}</td>
                  <td className="p-2 border-r border-slate-200">{cr.role}</td>
                  <td className="p-2 border-r border-slate-200">{cr.side}</td>
                  <td className="p-2">{cr.phone}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {/* 3. Media HDD Storage & Raw Data Backup */}
        <div className="space-y-2">
          <h3 className="text-xs font-black text-slate-900 uppercase tracking-wider border-b border-slate-300 pb-1">
            3. Media HDD Storage & Backup Logs
          </h3>
          <table className="w-full text-xs text-left border border-slate-300">
            <thead className="bg-slate-100 font-bold border-b border-slate-300">
              <tr>
                <th className="p-2 border-r border-slate-300">Ceremony</th>
                <th className="p-2 border-r border-slate-300">HDD Storage Drive</th>
                <th className="p-2 border-r border-slate-300">Folder Path</th>
                <th className="p-2">Copied By & Date</th>
              </tr>
            </thead>
            <tbody>
              {client.events.flatMap(ev => (ev.hddStorage || []).map(hdd => ({ ...hdd, eventName: ev.type }))).map((hdd, idx) => (
                <tr key={idx} className="border-b border-slate-200">
                  <td className="p-2 font-bold border-r border-slate-200">{hdd.eventName}</td>
                  <td className="p-2 border-r border-slate-200 font-bold">{hdd.hddName}</td>
                  <td className="p-2 border-r border-slate-200 font-mono text-[10px]">{hdd.folderPath}</td>
                  <td className="p-2">{hdd.copiedBy} ({hdd.copiedDate || 'N/A'})</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {/* 4. Production Tasks & Deliverables */}
        <div className="space-y-2">
          <h3 className="text-xs font-black text-slate-900 uppercase tracking-wider border-b border-slate-300 pb-1">
            4. Editing & Production Deliverables Tracking
          </h3>
          <table className="w-full text-xs text-left border border-slate-300">
            <thead className="bg-slate-100 font-bold border-b border-slate-300">
              <tr>
              ...
              {clientAssignments.map(task => {
                ...
                return (
                  <tr key={task.id} className="border-b border-slate-200">
                    <td className="p-2 font-bold border-r border-slate-200">{task.title}</td>
                    <td className="p-2 border-r border-slate-200">{assignee?.name || 'Unassigned'}</td>
                    <td className="p-2 border-r border-slate-200 font-extrabold">{task.status}</td>
        ...
        <div className="border-t border-slate-300 pt-4 text-center text-[10px] text-slate-500">
          <p>Convex Entertainments • Production Management System • Official Studio Brief Document</p>
        </div>
      </div>
    </div>
  );
